...
void handle_game_state(const signalr::value &m, signalr::hub_connection &connection, std::promise<void> &startTask) {
...
void handle_bot_action(signalr::hub_connection &connection, std::promise<void> &startTask) {
    ...
    // Get next action
    PlayerCommand nextAction = botService.getPlayerCommand();
...
void run() {
    ...
    connection.on("ReceiveGameState", [](const signalr::value &m) {
        handle_game_state(m, connection, startTask);
    });
